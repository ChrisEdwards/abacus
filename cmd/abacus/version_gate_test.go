...
func TestHandleVersionCheckResultTooOld(t *testing.T) {
	var buf bytes.Buffer
	info := beads.VersionInfo{Bin: "bd", Installed: "v0.25.0", Required: "v0.30.0"}
	...
	if !strings.Contains(out, "v0.25.0") || !strings.Contains(out, "v0.30.0") {
...
func TestHandleVersionCheckResultWarning(t *testing.T) {
	var buf bytes.Buffer
	info := beads.VersionInfo{Bin: "bd", Required: "v0.30.0"}
