...
func TestGetHelpSections(t *testing.T) {
	...
	t.Run("ActionsHas6Rows", func(t *testing.T) {
		if len(sections[1].rows) != 6 {
			t.Errorf("Actions section: expected 6 rows, got %d", len(sections[1].rows))
		}
	})

	t.Run("BeadActionsHas6Rows", func(t *testing.T) {
		if len(sections[2].rows) != 6 {
			t.Errorf("Bead Actions section: expected 6 rows, got %d", len(sections[2].rows))
		}
	})
