...
// For read operations, use brSQLiteClient which reads directly from SQLite.
...
// Use NewBrSQLiteClient for full Client functionality (reads via SQLite, writes via CLI).
...
func (c *brCLIClient) UpdatePriority(ctx context.Context, issueID string, priority int) error {
	if strings.TrimSpace(issueID) == "" {
