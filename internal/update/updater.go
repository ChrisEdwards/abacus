...
func (u *Updater) Update(ctx context.Context, version string) error {
	...
	tempFile, err := u.downloadRelease(ctx, version)
	if err != nil {
		return err
	}
...
// downloadRelease downloads the release binary to a temp file.
func (u *Updater) downloadRelease(ctx context.Context, version string) (string, error) {
	...
	tempFile, err := os.CreateTemp(filepath.Dir(execPath), "bv-update-*")
	if err != nil {
